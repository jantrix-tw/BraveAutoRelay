...
find ~/.mozilla/firefox \( -name "lock" -o -name ".parentlock" \) 2>/dev/null | while read -r f; do
    mv "$f" /tmp/ 2>/dev/null && echo "$LOG_PREFIX Cleared: $f"
done
find ~/.firefox-claude-monitor \( -name "lock" -o -name ".parentlock" \) 2>/dev/null | while read -r f; do
    mv "$f" /tmp/ 2>/dev/null && echo "$LOG_PREFIX Cleared claude-monitor lock: $f"
done
